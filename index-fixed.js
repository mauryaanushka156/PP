...
// Test route - verify server is working
app.get('/test', (req, res) => {
  res.json({ status: 'ok', message: 'Server is working!' });
});

// Try to load database, but don't fail if it doesn't work
let db = null;
try {
  db = require('./database');
} catch (error) {
  console.warn('Database module not available, running in API-only mode');
}
  ...
  app.listen(PORT, (err) => {
    if (err) {
      ...
      if (err.code === 'EADDRINUSE') {
        ...
        console.error('\nSOLUTIONS:');
        console.error(`1. Change PORT in server/index-fixed.js (line 5) to 3001`);
